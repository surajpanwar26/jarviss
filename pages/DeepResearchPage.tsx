  ...
  return (
    <div className="h-full flex flex-col p-4 animate-fade-in max-w-[1800px] mx-auto">
       ...
       <div className="flex-1 grid grid-cols-12 gap-4 overflow-hidden min-h-0">
          ...
          {/* Col 1: Logs & Chat (Left) */}
          <div className="col-span-3 flex flex-col gap-4 overflow-hidden">
             {/* Logs */}
             <div className="h-1/2 bg-black/20 rounded-xl border border-white/10 p-3 flex flex-col overflow-hidden relative">
                <h3 className="text-[10px] font-bold text-cyan-500 uppercase tracking-widest mb-2 flex items-center">
          ...
          {/* Col 2: Report / Images (Middle - Main Stage) */}
                      ...
                      activeTab === 'report' ? (
                        <>
                          {(status === ResearchStatus.PLANNING || status === ResearchStatus.SEARCHING || status === ResearchStatus.SYNTHESIZING) && !streamedReport ? (
                            <WaveLoader message="Generating Deep Research Report..." />
                          ) : (
                            <>
